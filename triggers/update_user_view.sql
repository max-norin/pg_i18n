...
BEGIN
    ...
    END LOOP;
    EXECUTE format('UPDATE %1s SET (%2s)=ROW(%3s) WHERE (%4s)=(%5s);', "lbt_table", array_to_string("lbt_columns", ','), array_to_string("lbt_values", ','), array_to_string("lbt_pk_columns", ','), array_to_string("lbt_pk_values", ','));
    RETURN NEW;
END
$$
LANGUAGE plpgsql
