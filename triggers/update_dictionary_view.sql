...
BEGIN
    ...
    END LOOP;
    EXECUTE format('UPDATE %1s SET (%2s)=ROW(%3s) WHERE (%4s)=(%5s) RETURNING to_json(%4s.*);', "lb_table", array_to_string("lb_columns", ','), array_to_string("lb_values", ','), array_to_string("lb_pk_columns", ','), array_to_string("lb_pk_values", ','), "lb_table") INTO "lb_record";
    ...
    EXECUTE format('
        INSERT INTO %1s (%2s) VALUES (%3s)
            ...
            DO UPDATE SET (%5s)=ROW(%6s);', "lbt_table", array_to_string("lbt_pk_columns" || "lbt_columns", ','), array_to_string("lbt_pk_values" || "lbt_values", ','), "lbt_pk_name", array_to_string("lbt_columns", ','), array_to_string("lbt_values", ','));
    RETURN NEW;
END
$$
LANGUAGE plpgsql
