...
DECLARE
    ...
    "lbt_pk_columns" CONSTANT TEXT[]   NOT NULL = public.get_primary_key("lbt_table");
    -- все колонки кроме primary_key (secondary key)
    "lbt_sk_columns" CONSTANT TEXT[]   NOT NULL = public.get_columns("lbt_table", FALSE) OPERATOR ( public.- ) "lbt_pk_columns";
    -- lang base
    ...
    "lb_ch_columns"  CONSTANT TEXT[]   NOT NULL = public.get_columns("lb_table", FALSE) OPERATOR ( public.- ) "lbt_sk_columns";
BEGIN
    -- insert or update and return record from lb_table
    "lb_record" = public.update_using_records("lb_table", "lb_ch_columns", OLD, NEW);
    -- insert or update and return record from lbt_table
    "lbt_record" = public.insert_or_update_using_records("lbt_table", NEW);
