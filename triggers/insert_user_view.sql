...
DECLARE
    ...
    "lb_columns"    CONSTANT TEXT[] NOT NULL   = array_except(get_columns("lb_table"), "lb_pk_columns");
    "lb_values"              TEXT[];
    ...
    "column"                 TEXT;
BEGIN
    FOREACH "column" IN ARRAY "lb_pk_columns"
        LOOP
    ...
    EXECUTE format('INSERT INTO %1s (%2s) VALUES (%3s) RETURNING to_json(%4s.*);',
                   "lb_table", array_to_string("lb_pk_columns" || "lb_columns", ','), array_to_string("lb_values", ','),
                   "lb_table")
        INTO "lb_record";
    ...
    EXECUTE format('INSERT INTO %1s (%2s) VALUES (%3s);',
                   "lbt_table", array_to_string("lbt_columns", ','), array_to_string("lbt_values", ','));
